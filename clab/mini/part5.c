...
list_find(struct list_node *head, int value, struct list_node **predp)
{
	...
	*predp = NULL; 
	...
	//temporary list_node pointer temp to iterate through linked list 
	struct list_node *temp = head; 

	//iterate through list until we reach last node 
	while (temp != NULL) {
		if (temp->value == value) {
			return temp; //return pointer when we found the node with value we are looking for 
		}
		*predp = temp; //set predp pointer point to the node we found 
		temp = temp->next; //start next iteration from next node 
	}

	//if value not found 
	if (temp == NULL) {
		//set predp pointer to NULL
		*predp = NULL;
		//return temp;
	} 
	//return NULL 
...
list_remove(struct list_node **headp, int value)
{
	...
	// TODO: Your code here.
	struct list_node *prev; //previous pointer to pass into find function 
	struct list_node *remove = list_find(*headp, value, &prev); //find node to remove 
	if (remove != NULL) { //if node to remove exists 
		if (prev == NULL) { //if it is the head node
			*headp = remove->next; //head pointer points to next node 
			free_node(remove); //free the node to remove 
		}
		else {
			prev->next = remove->next; //prev pointer skips over removed node 
			free_node(remove); //free the node to remove
		}
		return 1; 
	}
